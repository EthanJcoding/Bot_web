...
const Member = () => {
  ...
  const renderMemberCard = (member: Interfaces.Member) => {
    return (
      <div
        className="flex justify-between items-center"
        key={member.gameUsername}
      >
          ...
          {member.acs > 0 && member.tier !== '' && (
            <>
              <Image
                ...
                className="w-10 h-10"
              />
              <div className="text-sm text-muted-foreground ">{member.acs}</div>
            </>
          )}
          <AcsInputDialog
            gameUsername={member.gameUsername}
            edit={member.acs > 0 && member.tier !== ''}
          />
        </div>
      </div>
    )
  ...
  return <>{membersCard.map((member) => renderMemberCard(member))}</>
}
